    ...
    public async Task ShowTopAsync(CCSPlayerController requester)
    {
        try
        {
            ...
            while (await reader.ReadAsync().ConfigureAwait(false))
            {
                ...
                var deaths = reader.GetInt64(2);
                var kd = deaths > 0 ? (double)kills / deaths : (double)kills;
                requester.PrintToChat($" {ChatColors.Yellow}#{pos}  {ChatColors.Green}{name}  {ChatColors.White}K: {ChatColors.Green}{kills}  {ChatColors.White}K/D: {ChatColors.Green}{kd:F2}");
                pos++;
            }
