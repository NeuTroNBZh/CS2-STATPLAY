...
### CounterStrikeSharp Getting Started (Install Path Confirmation)
- URL: https://docs.cssharp.dev/docs/guides/getting-started.html
- Usage: reverifier le chemin d'installation exact apres un symptome de plugin non charge sur Dathost.
- Confirmed:
  - Metamod et CounterStrikeSharp doivent etre copies sous `game/csgo/addons/`.
  - Le troubleshooting officiel recommande de verifier le layout du dossier `game/csgo/addons` et `meta list`.

### CounterStrikeSharp PluginManager discovery convention
- URL: https://github.com/roflmuffin/CounterStrikeSharp
- Usage: comprendre pourquoi le plugin n'apparaissait pas dans `css_plugins list`.
- Confirmed:
  - le loader auto-scan les sous-dossiers de `plugins/` et cherche explicitement `Path.Combine(currentDir, dirName + ".dll")`.
  - un dossier `CS2Stats` avec un DLL principal `CS2Stats.Plugin.dll` n'est donc pas auto-decouvert.

### MySQL error semantics (operational handling)
- URL: https://dev.mysql.com/doc/mysql-errors/8.0/en/server-error-reference.html
- Usage: classifier les erreurs SQL de privilege pour rendre l'init DB plus robuste en environnement heberge.
- Confirmed:
  - 1044 correspond a un acces refuse sur la base.
  - 1227 correspond a un manque de privilege sur une commande.
  - ces cas doivent etre traites comme "tentative de fallback" vers une base existante plutot qu'echec immediat du plugin.

## 2026-03-27 (Suite)

### Open Source Initiative - MIT License
- URL: https://opensource.org/license/mit/
- Usage: verifier le texte canonique de la licence MIT pour une publication GitHub propre de la release 1.0.0.
- Confirmed:
  - le texte MIT standard couvre les droits de reutilisation, distribution et limitation de responsabilite attendus pour le projet.

### Internal schema and aggregation references
- Files:
  - `sql/001_v1_baseline_schema.sql`
  - `sql/002_v1_aggregation_stored_procedures.sql`
- Usage: construire une documentation data exhaustive et operationnelle pour devs web/API/IA.
- Confirmed:
  - le schema couvre joueurs, sessions, maps, rounds, events, snapshots de presence et agregats.
  - les procedures d'aggregation permettent de recalculer les niveaux lifetime/session/map de maniere explicite.
